...
from app.core.security import create_access_token, hash_password, is_active_account, user_roles, validate_password_length, verify_password
...
def register(db: Session, request: RegisterRequest) -> dict:
    try:
        ...
        db.rollback()
        logger.exception("Integrity error while registering user email=%s.", request.email)
        raise AppError("Email này đã được đăng ký.", 400) from exc
    except SQLAlchemyError as exc:
        db.rollback()
        logger.exception("Database error while registering user email=%s.", request.email)
        raise AppError("Lỗi cơ sở dữ liệu khi đăng ký tài khoản.", 500) from exc


def login(db: Session, request: LoginRequest) -> dict:
    try:
        identity = request.email.strip().lower()
        user = db.execute(
            select(User)
            .options(selectinload(User.user_roles).selectinload(UserRole.role))
            .where((func.lower(User.email) == identity) | (func.lower(User.username) == identity))
        ).scalar_one_or_none()
    except SQLAlchemyError as exc:
        logger.exception("Database error while logging in email=%s.", request.email)
        raise AppError("Lỗi cơ sở dữ liệu khi đăng nhập.", 500) from exc

    if user is None or not is_active_account(user.account_status) or not verify_password(request.password, user.password_hash):
