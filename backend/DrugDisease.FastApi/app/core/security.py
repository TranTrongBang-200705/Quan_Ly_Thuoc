...
ACTIVE_ACCOUNT_STATUSES = {"hoạt động", "hoat dong", "ho?t d?ng", "active"}
...
def is_active_account(status: str | None) -> bool:
    if not status:
        return False
    return status.strip().casefold() in ACTIVE_ACCOUNT_STATUSES
...
def get_current_user(
    ...
    if user is None or not is_active_account(user.account_status):
