...
async def app_error_handler(request: Request, exc: AppError) -> JSONResponse:
    if exc.status_code >= 500 and isinstance(exc.__cause__, SQLAlchemyError):
        logger.error(
            "Database error while handling %s %s",
            request.method,
            request.url.path,
            exc_info=(type(exc.__cause__), exc.__cause__, exc.__cause__.__traceback__),
        )
...
async def db_error_handler(request: Request, exc: SQLAlchemyError) -> JSONResponse:
    ...
    return JSONResponse(status_code=500, content={"message": "Lỗi cơ sở dữ liệu. Kiểm tra DB_* trong .env và SQL Server.", "status_code": 500})
