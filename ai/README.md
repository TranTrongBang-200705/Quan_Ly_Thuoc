...
AI folder also exposes a runtime service for the FastAPI backend:
...
- `GET /health`: reports service status and `modelLoaded` without crashing when the model file is missing.
- `GET /model/info`: returns model path/version/load state.
- `POST /predict/batch`: accepts candidate pairs with `thuocId`, `benhId`, and optional feature fields, then returns `diemDuDoan`, `nhanDuDoan`, and `nguonDiem = "AI_MODEL"`.

The default model path is `ai/models/random_forest_thuoc_benh.joblib`. Override it with `AI_MODEL_PATH` when needed.
